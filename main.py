...
import sqlite3
import requests
from knopka import asosiymenubutton, maxsulotlarbutoon,orqaga

api = '7123559250:AAHvtqHpZXoSrWoWuAwLubzoM44_FHddzYQ'
...
async def getitem(callback: CallbackQuery):
    ...
    await bot.send_photo(chat_id=chatid, caption=text, photo=image, reply_markup=orqaga(categry))
...
async def getitem(callback: CallbackQuery):
    ...
    await bot.delete_message(chat_id=chatid, message_id=callback.message.message_id)
    await bot.send_message(chat_id=chatid, text=category, reply_markup=orqaga(category))
